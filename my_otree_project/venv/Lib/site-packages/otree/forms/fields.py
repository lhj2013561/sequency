...
from otree.i18n import format_number_for_form
...
class FloatField(wtfields.FloatField):
    ...
    def _value(self):
        if self.data is None:
            return ''
        return format_number_for_form(self.data, places=common.FULL_DECIMAL_PLACES)
...
class CurrencyField(wtfields.Field):
    ...
    def _value(self):
        if self.data is None:
            return ''
        return format_number_for_form(
            to_dec(self.data), places=common.FULL_DECIMAL_PLACES
        )


class DecimalFormField(wtfields.Field):

    def __init__(self, label=None, validators=None, unit=None, **kwargs):
        super().__init__(label, validators, **kwargs)
        self.unit = unit
        self.widget = wg.DecimalWidget(unit=unit)

    def process_formdata(self, valuelist):
        if valuelist and valuelist[0]:
            try:
                input_val = handle_localized_number_input(valuelist[0])
                # Round to form precision before creating DecimalUnit
                input_places = self.unit.input_places
                quant = Decimal('0.1') ** input_places
                rounded = Decimal(input_val).quantize(
                    quant, rounding=decimal.ROUND_HALF_UP
                )
                data = self.unit(rounded)
            except (decimal.InvalidOperation, ValueError):
                self.data = None
                _gt = self.gettext
                raise ValueError(_gt('Not a valid decimal value'))
    ...
    def _value(self):
        if self.data is None:
            return ''
        places = self.unit.input_places
        return format_number_for_form(to_dec(self.data), places=places)
