...
async def trial_payload_function(participant_code, page_name, msg):
    ...
    if trial and msg_type == 'response':
        server_id = trial.id
        client_id = msg['trial_id']
        if server_id != client_id:
            await send_error()
            if server_id > client_id:
                msg_mismatch = (
                    "Maybe the client is trying to submit the same trial twice."
                )
            else:
                msg_mismatch = "Maybe the client skipped submitting a trial."
            msg = (
                "Trials: server and client are out of sync. "
                f"Server is expecting trial {server_id}, but client sent response for trial {client_id}. "
        ...
        if hasattr(PageClass, 'evaluate_trial'):
            try:
                feedback = PageClass.evaluate_trial(player, trial, response)
            except Exception:
                await send_error()
                raise
        else:
