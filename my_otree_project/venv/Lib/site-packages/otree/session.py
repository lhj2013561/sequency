...
def create_session(
    ...
    *,
    num_participants,
    progress_message_fxn=noop_progress_message,
    label='',
    room_name=None,
    is_mturk=False,
    is_demo=False,
    ...
    try:
        ...
        db.commit()
        progress_message_fxn("Created participants")
        ...
        for app_name in session_config['app_sequence']:
            progress_message_fxn(f"Creating models for {app_name}")
        ...
        progress_message_fxn("Running your creating_session functions")
        run_creating_session_functions(session, progress_message_fxn)
...
def run_creating_session_functions(session, progress_message_fxn):
    session_config = session.config
    for app in session_config['app_sequence']:
        models_module = get_models_module(app)
        target = models_module.Subsession.get_user_defined_target()
        func = getattr(target, 'creating_session', None)
        if func:
            progress_message_fxn(f"{app}.creating_session()")
            app_subsessions = models_module.Subsession.objects_filter(
                session=session
            ).order_by('round_number')
            for subsession in app_subsessions:
                func(subsession)
