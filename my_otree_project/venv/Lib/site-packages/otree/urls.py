...
ALWAYS_UNRESTRICTED = {
    # REST views don't need to be here because they inherit directly from HTTPEndpoint
    # and don't check the _requires_login flag to begin with.
    # it doesn't matter if that attr is set to True.
    'AssignVisitorToRoom',
...
UNRESTRICTED_IN_DEMO_MODE = ALWAYS_UNRESTRICTED.union(
    {
        ...
        'SessionData',
        'SessionDataAjax',
        'SessionMonitorAskForPresenceIcons',
        'SessionStartLinks',
        'WSCreateDemoSession',
        'WSSessionMonitor',
    }
...
# View names that should NOT use the global lock (long-running operations)
VIEWS_WITHOUT_LOCK = {
    'SessionDataAjax',
    'ExportWide',
    'ExportApp',
...
def get_paths_without_lock():
    """
    Extract URL path prefixes for views that should not use the global lock.
    Called after routes are built to avoid duplicating URL patterns.
    """
    paths = set()
    for route in routes:
        route_name = getattr(route, 'name', None)
        if route_name in VIEWS_WITHOUT_LOCK:
            # Extract path prefix before first parameter
            # e.g., "/session_data/{code}" -> "/session_data/"
