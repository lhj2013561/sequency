...
from otree.currency import BaseCurrency
...
class BaseFormFieldPartNode(Node):
    def process_token(self, token):
        try:
    ...
    def wrender(self, context):
        ...
        return self.render_field(fld)


@register('formfield_errors')
class FieldErrorsNode(BaseFormFieldPartNode):
    def render_field(self, fld):
        if not fld.errors:
            return ''
...
#     """
#     Can be misleading.
#     For checkboxes and radios, this gives the value, not whether it's checked.
#     What is the benefit of this compared to {{ form.xyz }}?
#     -   Can be used for sliders and other custom widgets.
#     -
#     """
#
#     def render_field(self, fld):
...
# use a script instead of <button type="submit" formnovalidate ...>,
# because we don't want pressing enter to submit the back button
@register('back_button')
class BackButton(Node):
    # can't use formnovalidate because it's not supported on safari
    def wrender(self, context):
        # Translators: the text of the 'back' button
        back_btn_text = gettext('Back')
        # return """<button type="submit" formnovalidate>Submit without validation</button>"""
        back_button_html = f'''
        <p>
            <button type="button" onclick="back_button()" class="otree-btn-back btn btn-secondary">{back_btn_text}</button>
        </p>
        '''
        return back_button_html
...
def localize(v):
    if isinstance(v, DecimalUnit):
        # NEW: Use unit-based formatting
        return v._format_for_display()
    if isinstance(v, BaseCurrency):
        # Existing Currency handling
        return v._format_currency()
