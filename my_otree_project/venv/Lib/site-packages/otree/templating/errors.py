...
class TemplateRenderingError(ErrorWithToken):
    def __str__(self):
        msg = super().__str__()
        if self.__cause__:
            # this is needed by render_template() public API.
            # otherwise people might miss the original error.
            # TemplateRenderingError is never actually displayed on ibis error page.
            # so we can write it however we want here.
            msg += f" (see previous error for specific details)"
        return msg
