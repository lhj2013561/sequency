...
OTREE_CONTENT_MARKER = 'class="_otree-content"'
...
class HtmlString(str):
    def truncated(self):
        ...
        # it could be 'no more pages left'.
        # work around that because printing should never raise an error.
        if OTREE_CONTENT_MARKER in self:
            i = self.index(OTREE_CONTENT_MARKER) + len(OTREE_CONTENT_MARKER)
            return '...' + self[i:]
        return self
