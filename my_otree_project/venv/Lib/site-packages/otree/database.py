...
from otree.common import expand_choice_tuples, get_models_module, inspect_field_names
...
def session_scope():
    ...
    Automatically creates a new session, commits on success, rolls back on error,
    and closes the session. Works correctly across both async tasks (via contextvars)
    and worker threads.
    ...
    Usage in background threads (e.g., session creation):
        def background_task():
            with session_scope():
                # Database operations here

        await loop.run_in_executor(None, background_task)
    """
...
class DBWrapper:
    ...
    3. uses context variables so each thread/async task gets its own DB session
    """

    def __init__(self):
        # Context variables work for both threads and async contexts
        self._context_var = contextvars.ContextVar('db_session', default=None)
    ...
    def get_or_404(
        self, Model, msg='Not found (maybe the session was deleted)', **kwargs
    ):
        try:
...
def get_engine():
    if IN_MEMORY:
        ...
        # Need to change this so that it doesn't throw when doing startapp, etc.
        # try:
        #     DATABASE_URL = os.environ['DATABASE_URL']
        # except KeyError:
        #     sys.exit(
        #         """DATABASE_URL environment variable is missing. """
        #         "If you want to use SQLite, you can set it to sqlite:///db.sqlite3."
        #     )
        kwargs = {}
    ...
    if _engine.url.get_backend_name() == 'sqlite':
        ...
        event.listen(
            _engine, 'connect', lambda c, _: c.execute('pragma foreign_keys=on')
        )
    return _engine
...
class DecimalFieldType(types.TypeDecorator):
    impl = types.Text()
    ...
    def process_result_value(self, value, dialect):
        """Convert to DecimalUnit with unit"""
        if value is None:
            return None
...
class SSPPGModel(AnyModel):
    ...
    _setattr_datatypes = {
        ...
        DecimalFieldType: (DecimalUnit, NoneType, int, float, Decimal),
...
def MultiStringField(**kwargs):
    if 'widget' in kwargs:
        raise ValueError('MultiStringField have widget= argument')
    return StringField(**kwargs, multiple=True)
...
def _setup_deferred_properties(mapper, cls):
    """Listen for finished mappers and apply DeferredProp
    configurations."""

    from otree.trials import BaseTrial

    for key, value in list(cls.__dict__.items()):
        if isinstance(value, Link):
            if issubclass(cls, BaseTrial) and key == 'player':
                sys.exit(
                    "Trial classes should not define 'player' field. "
                    "That is already defined automatically."
                )
            value._config(cls, key)
    # if issubclass(cls, ExtraModel):
    #     # TODO: currently has no effect because we don't check if it's frozen.
    #     cls.freeze_setattr()


EXTRAMODEL_FIELDS = dir(ExtraModel)
