...
def get_session_urls(session: Session, request: Request) -> dict:
    d = dict(
        session_wide_url=request.url_for(
            'SessionWideLink', anonymous_code=session._anonymous_code
        ),
...
class RESTGetSessionInfo(BaseRESTView):
    ...
    def post(self, participant_labels=None, participant_vars=None, session_vars=None):
        ...
        session_config_filtered = {}
        for k in session.config:
            try:
                json_dumps(session.config[k])
            except TypeError:
                pass
            else:
                session_config_filtered[k] = session.config[k]

        payload.update(
            config=session_config_filtered,
