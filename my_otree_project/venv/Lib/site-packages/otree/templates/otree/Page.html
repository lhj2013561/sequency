...
{% block body_main %}
    ...
    <script>
    // Shows alert if it exists (for errors after debug_info loads).
    // Also sets flag which debug_info checks (for errors before it loads).
...
{% block trial %}
  {% if has_trial %}
    ...
    <script>let _trialSocketUrl = "{{ view.trial_url() }}";</script>
    {% comment %} <script src="{% static 'otree/js/otree-front.js' %}"></script>
    <script src="{% static 'otree/js/trials-core.js' %}"></script> {% endcomment %}
  {% endif %}
{% endblock %}