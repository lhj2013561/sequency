...
{% block head_title %}{% if is_archive %}Archived {% endif %}Sessions{% endblock %}
{% block title %}{% if is_archive %}Archived {% endif %}Sessions{% endblock %}
      ...
      <table class="table">
        ...
        {% for s in sessions %}
          <tr {% if s.archived %}class="hidden"{% endif %}>
            ...
            <td>{{ s.config.name }}</td>
            <td class="created-date" data-timestamp="{{ s._created }}"></td>
            <td>{{ s.label }}</td>
            <td>
...
// if all checkboxes are selected check also "checkAll" checkbox
      ...
      // Format timestamps as YYYY-MM-DD HH:MM in user's timezone
      document.querySelectorAll('.created-date').forEach(function(el) {
          let timestamp = parseInt(el.dataset.timestamp);
