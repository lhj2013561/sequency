...
class Session(MixinVars, otree.database.SSPPGModel):
    ...
    def _created_readable(self):
        """i guess we use this because we don't have the timezone"""
        now = time.time()
    ...
    def advance_selected_participants(self, participants):
    ...
    def advance_last_place_participants(self, visited_only=False):
        # note-to-self
        ...
        pp_set = self.pp_set
        if visited_only:
            pp_set = pp_set.filter_by(visited=True)
        participants = list(pp_set.order_by('id_in_session'))
        ...
        self.advance_selected_participants(last_place_participants)
